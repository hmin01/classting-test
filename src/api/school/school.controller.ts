import { Body, Controller, Req, UsePipes, ValidationPipe } from '@nestjs/common';
// DTO
...
export class SchoolController {
  ...
  @ApiOperation({ summary: '학교 페이지 생성', description: '지역과 학교명을 이용하여 학교 페이지를 생성합니다.' })
  ...
  @UsePipes(ValidationPipe)
  @ApiBearerAuth()
  @ApiOperation({ summary: '학교 페이지 구독', description: '학교 페이지를 구독합니다.' })
  @ApiOkResponse({ description: '구독 완료' })
  @ApiUnauthorizedResponse({ description: '권한 없음' })
  async subscribe(@Body() subscribeDto: SubscriptionDto, @Req() req: any): Promise<Date> {
  ...
  @UsePipes(ValidationPipe)
  @ApiBearerAuth()
  @ApiOperation({ summary: '학교 페이지 구독 취소', description: '구독 중인 학교 페이지를 취소합니다.' })
  @ApiOkResponse({ description: '구독 취소 완료' })
  @ApiNotFoundResponse({ description: '구독 정보 없음' })
  @ApiUnauthorizedResponse({ description: '권한 없음' })
  async unsubscribe(@Body() subscribeDto: SubscriptionDto, @Req() req: any): Promise<Date> {
    return this.subscriptionService.unsubscribe(req.user, subscribeDto.school_id);
  }
}
